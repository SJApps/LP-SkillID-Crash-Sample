...
class ViewController: UIViewController, LPMessagingSDKdelegate {
    ...
    @IBOutlet var skillIDTextField: UITextField!
    ...
    override func viewDidLoad() {
        ...
        LPMessagingSDK.instance.subscribeLogEvents(LogLevel.trace) { (log) -> () in
            print("LPMessagingSDK log: \(log.text)")
        }
    }
    ...
    */
    @IBAction func showConversation() {
        ...
        let skillID = self.skillIDTextField.text

        if skillID!.length > 0 {
            self.conversationQuery = LPMessagingSDK.instance.getConversationBrandAndSkillQuery(account!, skillID: skillID!)
        } else {
